...
use prime_skill_native_core::{
    native_bootstrap_status as build_native_bootstrap_status, preview_install_target as native_preview_install_target,
    InstallManifest, InstallProgressEvent, LocalInstallRecord, LocalInstallRegistry, PreviewInstallTarget,
    ...
    ListToolInstancesPayload {
        ...
        items: vec![
            ...
            ToolInstanceDiscoveryPayload {
                ...
                os_type: "windows".to_string(),
                detected_install_path: Some("C:/Users/Prime/AppData/Roaming/npm/opencode.cmd".to_string()),
...
#[tauri::command]
fn select_workspace(input: Option<SelectWorkspaceInput>) -> Result<WorkspaceSelectionPayload, String> {
    let selected = input
...
#[tauri::command]
fn preview_install_target(input: PreviewInstallTargetInput) -> Result<PreviewInstallTarget, String> {
...
#[tauri::command]
async fn apply_install_ticket(app: AppHandle, input: ApplyInstallTicketInput) -> Result<ApplyInstallTicketResponse, String> {
    let manifest = fetch_manifest(&input).await?;
    ensure_supported_manifest(&manifest)?;

    let registry_path = local_registry_path(&app);
    let apply_result = prime_skill_native_core::execute_install(
        &manifest,
        &registry_path,
        &input.trace_id,
    ...
    for stage in ["ticket_issued", "downloading", "staging", "verifying", "committing"] {
...
async fn fetch_manifest(input: &ApplyInstallTicketInput) -> Result<InstallManifest, String> {
    ...
    if !response.status().is_success() {
        return Err(format!("manifest request failed with {}", response.status()));
    }
...
async fn consume_stage(
    ...
    if !response.status().is_success() {
        return Err(format!("consume request for {stage} failed with {}", response.status()));
    }
